...
from ssb_project_cli.ssb_project.build.poetry import is_source_not_set_in_lock_file
...
def test_is_source_not_set_in_lock_file(
    isfile: bool, data: str, expected: bool
) -> None:
    ...
    with patch(f"{POETRY}.os.path.isfile", return_value=isfile):
        with patch(f"{POETRY}.open", mock_open(read_data=data)):
            assert expected == is_source_not_set_in_lock_file(
